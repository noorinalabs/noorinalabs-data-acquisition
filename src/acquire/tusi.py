...
``tusi`` corpus), so the two never share a parser or a corpus
namespace. :mod:`src.parse.tusi` parses the clone.
...
TUSI_GITHUB_URL = "https://github.com/narmafraz/ThaqalaynData.git"
...
def run(raw_dir: Path) -> Path:
    """Clone ThaqalaynData and return the ``tusi`` raw directory.
    ...
    """
    dest = ensure_dir(raw_dir / "tusi")
    clone_root = dest / "clone"

    clone_repo(TUSI_GITHUB_URL, clone_root)
    ...
    logger.info("tusi_acquired", books=len(TARGET_BOOK_SLUGS), files=len(book_files))

    write_manifest(dest, book_files)
    emit_raw_new_for_manifest(source="tusi", local_dir=dest, files=book_files)
    return dest

...
_KIND_PATTERNS: dict[str, tuple[str, ...]] = {
    ...
    # gates built in noorinalabs-main#735 (worklist #4 + #1) and rolled into this
    # repo by #744. Each is a `.claude/lib/check_*.py` invoked identically by a
    # ci.yml job and a pre-commit hook; classifying them here is what makes the
    # sync-drift gate DEMAND the mirror (the #684 contract: an un-classified CI
    # check is a silent blind spot). Patterns match the script basename (present
    # on both sides' invoke line) and the hook id / job name.
